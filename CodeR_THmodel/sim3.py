...
widths=64
depths=4
list_n=np.ceil(np.linspace(2**8, 2**14, num=20)).astype(int).tolist()
noise_level=[0,0.2,0.4,0.6]
list_sim=[i for i in range(200)]

list_allset = list(product(*[list_n,noise_level,list_sim]))

n = list_allset[idx_data][0]
noise= list_allset[idx_data][1]
...
THE_SIM_MARK = "The_sample_size{}_noise_{}_sim_{}_".format(n,noise,sim)
