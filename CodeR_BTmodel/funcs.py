...
def generate_synthetic_dataset(n, d,model,true_weights,seed):
    ...
    if model == 'bt': # Bradley-Terry model
        ...
        r1 = 2*torch.sin(4*state_features2 @ true_weights)#Hg(4*state_features2 @ true_weights)#Hg(4*state_features2 @ true_weights)#
        r0 = -2*torch.sin(4*state_features2 @ true_weights)#Hg(4*state_features2 @ true_weights)#Hg(4*state_features2 @ true_weights)#
...
def generate_synthetic_dataset2(n, d, model, true_weights, seed, noise_rate):
    ...
    if model == 'bt': # Bradley-Terry model
        ...
        for k in range(d):
            state_features2[:,k] = torch.sin(state_features[:,k])
        r1 = 2*torch.sin(4*state_features2 @ true_weights)#Hg(4*state_features2 @ true_weights)#torch.sin(4*state_features2 @ true_weights)#
        r0 = -2*torch.sin(4*state_features2 @ true_weights)#Hg(4*state_features2 @ true_weights)#torch.sin(4*state_features2 @ true_weights)#
