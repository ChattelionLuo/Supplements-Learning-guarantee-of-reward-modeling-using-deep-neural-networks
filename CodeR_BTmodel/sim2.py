...
comparison_model= 'thurstonian'#thurstonian
...
for epoch in range(epochs):
    ...
    if current_loss < best_loss:
        ...
        trigger_times = 0
        torch.save(model.state_dict(), os.path.join("./model2/", "dCOR_idata_" + THE_SIM_MARK + '_net.pt'))
    else:
        trigger_times += 1
...
###read the best model
file_net_dict =  os.path.join("./model2/", "dCOR_idata_" + THE_SIM_MARK + '_net.pt')
...
dict_res = {#"pred_test":pred_test,
            "reg_test":reg_test}
import pickle
with open(os.path.join("./result2/",  "dCOR_idata_" + THE_SIM_MARK + '_res.pickle'), 'wb') as handle:
